...
let ident_msg = fun log timestamp name vs ->
  try
    if not (Hashtbl.mem aircrafts name) &&
      ...
      let _b = Pprzbus.bind name (fun _ args -> if timestamp then ac_msg_closure args.(1) args.(2) else ac_msg_closure "" args.(0))
...
let () =
  ...
  Pprzbus.init "Paparazzi server" "READY" (fun _ _ -> ());
